...
self.addEventListener('install', (event) => {
    if (DEBUG) console.log(`[SW ${SW_VERSION}] install event fired — calling skipWaiting()`);
...
self.addEventListener('activate', (event) => {
    if (DEBUG) console.log(`[SW ${SW_VERSION}] activate event fired — calling clients.claim()`);
    ...
    event.waitUntil(
        self.clients.claim().then(() => {
            if (DEBUG) console.log(`[SW ${SW_VERSION}] clients.claim() done — controlling all open clients`);
        })
    );
});
...
self.addEventListener('fetch', (event) => {
    ...
    if (!url.startsWith(self.location.origin)) {
        ...
        // if (DEBUG) console.log(`[SW] SKIP (cross-origin): ${url}`);
        return;
    }
